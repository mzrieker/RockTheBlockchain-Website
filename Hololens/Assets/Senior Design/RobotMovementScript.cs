...
namespace HoloToolkit.Unity.InputModule
{
	...
	/// </summary>
	public class RobotMovementScript : MonoBehaviour, IFocusable, IInputHandler, ISourceStateHandler
	{
		/// <summary>
		...
		private void Start()
		{
			...
			OriginalPosition = HostTransform.position;
		...
		private void Update()
		{
			...
			if (IsDraggingEnabled && !isDragging)
			{
				HostTransform.position = OriginalPosition;
			}
		}
		...
		private void UpdateDragging()
		{
            ...
            currentInputSource.TryGetPointerPosition(currentInputSourceId, out inputPosition);
				...
				//Debug.Log("Dragging Position: " + 10*(draggingPosition));
				//Debug.Log("Host Transform: " + 10 * HostTransform.position);

				if (10 * (HostTransform.position.x) < -5)
				{
					Debug.Log("Left");
					//#if NETFX_CORE
					//System.Threading.Tasks.Task.Delay(5000000);
					//#endif
					tcpServer.SendMessage("Left");
				}

				else if (10 * (HostTransform.position.x) > 5)
				{
					Debug.Log("Right");
					tcpServer.SendMessage("Right");
				}

				else if (10 * (HostTransform.position.y) < 7.5)
				{
					Debug.Log("Backwards");
					tcpServer.SendMessage("Backwards");
				}

				else if (10 * (HostTransform.position.y) > 12.5)
				{
					Debug.Log("Forward");
					tcpServer.SendMessage("Forward");
				}
